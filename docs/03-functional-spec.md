...
- ✅ **구현됨** 정독 화면 상단 **재생/일시정지 버튼**(청취 화면과 동일 토글). `playing` 상태는 App 이 단일 소스로 보유(Player 와 일원화 → 두 화면 상태 정합).
- ✅ **구현됨** 문장(요소) **더블클릭** → 그 청크부터 재생(seek + play). **단클릭**은 위치/하이라이트 이동만(재생 시작 안 함). 더블클릭은 대기 중 단클릭 seek 를 250ms 디바운스로 취소해, 실제 마우스 더블클릭의 연속 click 이 `seekToChunk` 를 연달아 호출하는 레이스(Supertonic 이 seek 마다 합성 취소·재시작 → 재생 씹힘)를 막는다. offset 획득은 클릭 요소의 `data-start`(원문 절대 오프셋) 기준.
